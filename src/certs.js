...
const fs = require('fs')
const pdfBase = './src/certificate.pdf'
...
async function generatePdf (profile, reasons) {
  ...
  //const existingPdfBytes = await fs.readFile(pdfBase).then((err,res) => res.arrayBuffer())

  const existingPdfBytes = await fs.readFileSync(pdfBase)

  const pdfDoc = await pdf_lib.PDFDocument.load(existingPdfBytes)
  const page1 = pdfDoc.getPages()[0]

  const font = await pdfDoc.embedFont(pdf_lib.StandardFonts.Helvetica)
